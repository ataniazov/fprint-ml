...
#define write_pixel(num) do { \
	...
	for (i = 0; i < nr_minutiae; i++) {
		struct fp_minutia *min = minlist[i];
		printf("x:%3d y:%3d ex:%3d ey:%3d dir:%2d rel:%f type:%2d aprng:%d ftr_id:%d nm_nbrs:%d\n", min->x, min->y, min->ex, min->ey, min->direction,
			min->direction,	min->reliability, min->type, min->appearing,
			min->feature_id, min->num_nbrs);
	}
}
