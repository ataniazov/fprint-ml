...
static GtkWidget *create_enroll_dialog(void) {
    ...
    edlg_dialog = gtk_dialog_new_with_buttons(
        tmp, GTK_WINDOW(mwin_window),
    ...
    gtk_dialog_set_response_sensitive(GTK_DIALOG(edlg_dialog), GTK_RESPONSE_OK,
                                      FALSE);
    return edlg_dialog;
}
...
static void __enroll_stopped(int result) {
    ...
    if (result < 0) {
        GtkWidget *dialog = gtk_message_dialog_new_with_markup(
            ...
            GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "Enroll failed with error %d",
            result, NULL);
        ...
        gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(edlg_progress_bar), 0.0);
        gtk_label_set_text(GTK_LABEL(edlg_instr_lbl), "Click Cancel to "
                                                      "continue.");
    } else if (result == FP_ENROLL_COMPLETE) {
        gtk_progress_bar_set_text(GTK_PROGRESS_BAR(edlg_progress_bar),
                                  "Enrollment complete!");
        gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(edlg_progress_bar), 1.0);
