...
static void iwin_activate_dev(void) {
    ...
    while ((print = fp_dscv_prints[i++])) {
        ...
        gtk_widget_set_sensitive(iwin_fing_checkbox[fnum], TRUE);
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(iwin_fing_checkbox[fnum]), TRUE);
    }

    if (fp_dev_supports_imaging(fpdev)) {
        int width = fp_dev_get_img_width(fpdev);
        int height = fp_dev_get_img_height(fpdev);
        gtk_widget_set_size_request(iwin_verify_img, (width == 0) ? 192 : width,
                                    (height == 0) ? 192 : height);
...
err:
    ...
    dialog = gtk_message_dialog_new_with_markup(
        GTK_WINDOW(mwin_window),
        GTK_DIALOG_DESTROY_WITH_PARENT | GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR,
        GTK_BUTTONS_OK, "Could not load print for %s, error %d", fingerstr(i),
        r, NULL);
    gtk_dialog_run(GTK_DIALOG(dialog));
    gtk_widget_destroy(dialog);
}
