...
static void vwin_refresh(void) {
    ...
    while ((print = fp_dscv_prints[i++])) {
        ...
        gtk_list_store_set(vwin_fingmodel, &iter, FC_COL_PRINT, print,
                           FC_COL_FINGSTR, fingerstr(fnum), FC_COL_FINGNUM,
                           fnum, -1);
    }
...
static void vwin_activate_dev(void) {
    ...
    if (fp_dev_supports_imaging(fpdev)) {
        ...
        gtk_widget_set_size_request(vwin_verify_img, (width == 0) ? 192 : width,
                                    (height == 0) ? 192 : height);
...
static void vwin_vfy_status_print_loaded(int status) {
    if (status == 0) {
        ...
        gchar *msg =
            g_strdup_printf("<b>Status:</b> Error %d, print corrupt?", status);
...
static void vwin_cb_fing_changed(GtkWidget *widget, gpointer user_data) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(vwin_fingmodel), &iter, FC_COL_PRINT,
                       &dprint, -1);
...
static GtkWidget *vwin_create(void) {
    ...
    vwin_fingmodel =
        gtk_list_store_new(3, G_TYPE_POINTER, G_TYPE_INT, G_TYPE_STRING);
    vwin_fingcombo =
    ...
    gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(vwin_fingmodel), 0,
                                    fing_sort, NULL, NULL);
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(vwin_fingmodel), 0,
                                         GTK_SORT_ASCENDING);
    ...
    gtk_box_pack_start(GTK_BOX(vwin_ctrl_vbox), vwin_radio_normal, FALSE, FALSE,
                       0);

    vwin_radio_bin = gtk_radio_button_new_with_label_from_widget(
        GTK_RADIO_BUTTON(vwin_radio_normal), "Binarized");
    gtk_box_pack_start(GTK_BOX(vwin_ctrl_vbox), vwin_radio_bin, FALSE, FALSE,
                       0);
    ...
    gtk_box_pack_end(GTK_BOX(vwin_ctrl_vbox), vwin_img_save_btn, FALSE, FALSE,
                     0);
