...
#define for_each_tab_call_op(op)                                               \
    do {                                                                       \
        int __fe_tabno;                                                        \
...
static GtkWidget *mwin_create_devbar(void) {
    ...
    mwin_devcombo = gtk_combo_box_new_with_model(GTK_TREE_MODEL(mwin_devmodel));
...
static void mwin_create(void) {
    ...
    gtk_window_set_title(GTK_WINDOW(mwin_window), "fPrint ML Project v1.0");
    ...
    for (i = 0; i < G_N_ELEMENTS(tabs); i++) {
        ...
        gtk_notebook_append_page(GTK_NOTEBOOK(mwin_notebook), tab->create(),
...
int main(int argc, char **argv) {
    ...
    gtk_init(&argc, &argv);
    gtk_window_set_default_icon_name("fprint_ml");
...
void pixbuf_destroy(guchar *pixels, gpointer data) { g_free(pixels); }
...
GdkPixbuf *img_to_pixbuf(struct fp_img *img) {
    ...
    return gdk_pixbuf_new_from_data(rgbdata, GDK_COLORSPACE_RGB, FALSE, 8,
                                    width, height, width * 3, pixbuf_destroy,
                                    NULL);
}
...
GtkWidget *create_scan_finger_dialog(void) {
    ...
    dialog = gtk_dialog_new_with_buttons(
        "Scan finger", GTK_WINDOW(mwin_window),
        GTK_DIALOG_MODAL | GTK_DIALOG_NO_SEPARATOR, GTK_STOCK_CANCEL,
